...
class LLMClient:
    ...
    async def generate_completion(
        ...
        # 1. Attempt Groq API (High-speed Qwen / GPT-OSS)
        if settings.GROQ_API_KEY:
                    ...
                    async with httpx.AsyncClient(timeout=12.0) as client:
                        ...
                        if resp.status_code == 200:
                            ...
                            if content and content.strip():
                                logger.info(f"Groq LLM ({model_name}) responded successfully.")
                                return content
                        else:
                            logger.warning(f"Groq {model_name} status {resp.status_code}: {resp.text[:150]}")
