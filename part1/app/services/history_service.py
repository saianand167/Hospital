...
class HistoryService:
    ...
    async def process_message(
        ...
        if is_touch_input and target_field and touch_value is not None:
            ...
            if target_field and is_negative_ans:
                ...
                # If target field was chief_complaint and not extracted, use patient_message
                if target_field == "chief_complaint" and not history.chief_complaint.text:
                    history.chief_complaint = ChiefComplaint(
                        text=patient_message.strip(),
                        canonical=patient_message.strip().lower()
                    )
    ...
    def _ensure_target_field_populated(cls, history: ClinicalHistoryJSON, target_field: Optional[str], text: str):
        ...
        if target_field == "chief_complaint":
            if not history.chief_complaint.text:
                history.chief_complaint = ChiefComplaint(
                    text=text.strip(),
                    canonical=t_low
                )
        elif target_field == "duration_days" and hpi_dict.get("duration_days") is None:
        ...
        elif target_field == "fever" and hpi_dict.get("fever") is None:
            hpi_dict["fever"] = False if any(w in t_low for w in ["no", "none", "లేదు", "లేవు", "नहीं"]) else True
        elif target_field == "cough" and hpi_dict.get("cough") is None:
            hpi_dict["cough"] = False if any(w in t_low for w in ["no", "none", "లేదు", "లేవు", "नहीं"]) else True
        elif target_field == "breathlessness" and hpi_dict.get("breathlessness") is None:
            hpi_dict["breathlessness"] = False if any(w in t_low for w in ["no", "none", "లేదు", "లేవు", "नहीं"]) else True
            ...
            if target_field not in hpi_dict or hpi_dict.get(target_field) is None:
                hpi_dict[target_field] = text
